# Bolide Luminosity Model (bolum) Version 7
# C.J. Miko — modified to v7 with Borovička τ, single luminosity term, per-fragment reporting,
# and multi-stage fragmentation (Feb 2026)
# Instructions: Modify inputs in the input deck (bolum_in.dat) and run
# This code is UNCLASSIFIED

# Recent Additions in v7:
...
def rdinput(inputpath):
    ...
    broadband_si       = params.get("broadband_si", 0)
    strength_secondary = params.get("strength_secondary", init_strength * 0.3) # default 30% of initial
    rho_tau_scale      = params["rho_tau_scale"]

    n_frag_init = n_fragments

    return (diameter, velocity, theta_deg, density, porosity, init_strength,\
            altstart, xstart, tstart, dt, tstop, n_fragments,\
            broadband_si, strength_secondary, rho_tau_scale, n_frag_init)

# writing out initial parameters to output file
def write_init_params(diameter, velocity, theta_deg, density, porosity, init_strength,
                      altstart, xstart, tstart, dt, tstop, n_fragments,
                      broadband_si, strength_secondary, rho_tau_scale, n_frag_init):

    init_params = open('init_params.dat', 'w')
...
def wr_out(bolide_outputs, luminosity, y, t, mass, diameter, area, KE, v, acc, q_h, Fdrag,
    ...
    bolide_outputs.write(f"KE = {KE:.4e}  v = {v:.4e}  acc = {acc:.4e}  E_rad_ttl = {E_rad_total:.4e}  E_dep = {E_deposited:.4e}  Fdrag = {Fdrag:.4e}  theta_deg = {theta_deg:.3f}\n")
    bolide_outputs.write(f"q_h = {q_h:.4e}  q = {q:.4e}  M = {M:.4e}  T_stag = {T_stag:.4e}  p_stag = {p_stag:.4e}  T_surf = {T_surf:.4e}\n")
    bolide_outputs.write(f"T = {T:.4e}  p = {p:.4e}  rho = {rho:.4e}  a = {a:.4e}  tau = {tau:.4e}  frag_stage = {frag_stage}\n\n")
...
def plot_outputs(basename, header, times, lums, alts, frag_times, frag_alts, velocities, accels, qs, strength,
    ...
    plt.gcf().text(.14, .74, f"Total Rad E = {E_rad_tot / jkt:.3e} kt", fontsize=10, color='black')
    ...
    # 2. luminosity in Watts vs time in log/linear scale
    ...
    # 3. radiant intensity in W/sr vs time in linear scale
    ...
    # 4. radiant intensity in W/sr vs time in log scale
    ...
    plt.gcf().text(.20, .89, header, fontsize=10, color='black')
    if frag_alts:
        plt.axhline(frag_alts[0], linestyle='--', color='green', alpha=0.6, label=f"Frag at {frag_alts[0]/1000.:.1f} km and {frag_times[0]:.2f} sec")
    ...
    plt.gcf().text(.20, .89, header, fontsize=10, color='black')
    if frag_alts:
        plt.axhline(frag_alts[0], linestyle='--', color='green', label=f"Frag at {frag_alts[0]/1000.:.1f} km and {frag_times[0]:.2f} sec")
    plt.gcf().text(.69, .74, f"Total Rad E = {E_rad_tot:.3e} J", fontsize=10, color='black')
    plt.gcf().text(.65, .71, f"Approx Event E = {E_event / jkt:.3e} kt", fontsize=10, color='black')
...
def tau_calc(v, mass, rho, rho_tau_scale, n_fragments):
    ...
    if rho_tau_scale == 1:
        tau = tau * (rho / 5.e-4)**0.67  # empirical term set to match Chelyabinsk luminosity
...
def frag_attempt(bolide_outputs, alt, t, frag_stage, frag_times, frag_alts,
    ...
    if q > current_strength:
        ...
        # Discrete + cloud for realism; gated to preserve small bolides
        # if frag_stage >= 1 and mass > 1.e5:  # Post-first frag, only for >~few-ton bodies
        #     # Pancake/cloud: area explodes (mimics 10^3-10^4 microfrags + dust)
        #     cloud_factor = min(6.0, 3.0 * (1 + 0.25 * frag_stage))  # Starts 6x, ramps to 25x
        #     area_new = cloud_factor * area_new
        #     # Ramp n_fragments for per-frag lum (keeps your reporting)
        #     n_fragments = int(n_fragments * (1 + 0.3 * frag_stage))
        #     # Cap to avoid infinite loop
        #     if frag_stage > 25 or mass < 5.e2:
        #         return False, area, mass, frag_stage, frag_times, frag_alts, n_fragments
...
def bolide_luminosity_model(diameter, velocity, theta_deg, zstart, xstart, density, porosity,
                            tstart, dt, tstop, n_fragments, broadband_si, strength_secondary,
    ...
    f"Strength: {strength_initial:.1e} → {strength_secondary:.1e} Pa")

    # open output files
    # general output file
    bolide_outputs = open('bolide_out.dat', 'w')
    bolide_outputs.write(f"Bolide Luminosity and Fragmentation Simulation — v7\n")
    ...
    bolide_pvt = open('power_vs_time.dat', 'w')
...
(diameter, velocity, theta_deg, density, porosity, init_strength, zstart, xstart,
tstart, dt, tstop, n_fragments, broadband_si, strength_secondary, rho_tau_scale, n_frag_init) = rdinput(inputpath)

# write out initial parameters
write_init_params(diameter, velocity, theta_deg, density, porosity, init_strength,
zstart, xstart, tstart, dt, tstop, n_fragments, broadband_si, strength_secondary,
rho_tau_scale, n_frag_init)

# run
(header, strength, T_array, p_array, rho_array, a_array, lum_array, alt_array, time_array,
mass_array, dia_array, area_array, frag_times, frag_alts, KE_array, v_array, accel_array,
heatflux_array, Fdrag_array, E_rad_ttl_array, q_array, Mach_array, T_stag_array,
p_stag_array, T_surf_array, peak_power, peak_rad_i, E_rad_tot, E_event) = bolide_luminosity_model(diameter, velocity, theta_deg, zstart, xstart,\
                                                                                                  density, porosity, tstart, dt, tstop, n_fragments,\
                                                                                                  broadband_si, strength_secondary, rho_tau_scale,
                                                                                                  n_frag_init)
...
plot_outputs(basename, header, time_array.flatten(), lum_array.flatten(), alt_array.flatten(),
             ...
             strength, T_stag_array.flatten(), p_stag_array.flatten(), T_surf_array.flatten(),
